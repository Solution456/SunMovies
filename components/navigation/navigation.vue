...
<template>
    ...
    <header class="page_header w-[1076px] h-full max-w-full relative px-4 xs:mx-auto">

    
    <nav class="flex h-full items-center justify-between">
        <div class="navigation__logo font-poppins font-bold text-xl xs:text-lg flex items-center justify-center h-full">
            <a :href="NAV_ROUTES.HOME['to']">s<span class=" text-yellow-300">u</span>n</a>
        </div>

        <div v-if="!active" class="h-full">
            <div class="navigation__items hidden font-poppins font-medium xs:block h-full ">
                <ul class="flex gap-2 h-full">
                    <li v-for="route in NAV_ROUTES" :key="route.name" class="navigation__item--link text-gray-400 hover:text-white hover:border-b hover:border-b-yellow-300 transition h-full">
        ...
        <div v-if="active" class="search-container max-w-[600px] m-auto flex flex-grow flex-shrink-0 basis-auto">
        ...
        <div class="navigation__actions flex items-center h-full">
            <NavigationActions @update="active=!active"></NavigationActions>
        </div>
    </nav>
</header>
</div>
